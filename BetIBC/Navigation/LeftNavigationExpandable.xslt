...
-->
    ...
    <xsl:if test="@AbsoluteLevel = 2">
      <input type="checkbox" class="expand-trigger js-remember-state" checked="">
		...
		<div>
      <xsl:if test="@AbsoluteLevel = 2">
			  <xsl:attribute name="class">expand-box expand-container</xsl:attribute>
			  <label>
          <xsl:attribute name="class">expand-box__header expand-container__btn</xsl:attribute>
      ...
      <xsl:if test="@AbsoluteLevel = 3">
        <xsl:attribute name="class">expand-box__list-item <xsl:if test="@Active='True'"> active </xsl:if> level-<xsl:value-of select="@AbsoluteLevel"></xsl:value-of>
        </xsl:attribute>
        <a>
          <xsl:attribute name="class">expand-box__list-item-link <xsl:if test="@Active='True'"> active </xsl:if> level-<xsl:value-of select="@AbsoluteLevel"></xsl:value-of>
          </xsl:attribute>
          <xsl:attribute name="href">
            <xsl:value-of select="@FriendlyHref" disable-output-escaping="yes"/>
          </xsl:attribute>
        ...
        <xsl:if test="@AbsoluteLevel = 2">
				  <div class="expand-box__content expand-container__content expand-container__content--limit-height">
